...
function showToggleSwitch(toggle) {
  actionEl.children[2].style.display = toggle ? "flex" : "none";
}
...
function assignSpeech(user) {
  try {
    ...
    toggleActiveUser(user?.userId, true);
    //hide toggle switch to prevent user from disabling speech
  } catch (err) {
