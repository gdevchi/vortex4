...
function getAudioStreamAccess(speechDisabled) {
  navigator.mediaDevices
    .getUserMedia({ audio: true })
    .then((stream) => {
      if (speechDisabled) {
        chanegMicStatus("Microphone feature disabled", false);
      } else {
      ...
      socket.emit("user-joined", { username, speechDisabled });
    })
...
window.addEventListener("DOMContentLoaded", () => {
  const speechDisabled = confirm("Do you want to listen only?");
  getAudioStreamAccess(speechDisabled);
});
