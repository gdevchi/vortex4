...
function calculateRemainingTime(assignedAt, delayed = 0) {
  ...
  const remainingSeconds = 30 - startedAtSeconds;
  return remainingSeconds + 1 > 30 ? 30 : remainingSeconds + 1;
}

function startSpeakerTimer(user) {
  var seconds = calculateRemainingTime(user.assignedAt, user.delayed); //calculate remaining seconds
...
function setAssignUser(user) {
  ...
  //Start Speaker Timer
  if (user?.userId) {
    //if (user?.userId === state?.user?.userId) user.assignedAt = Date.now();
