...
const speakerMic = document.querySelector("#speaker-mic");
const timer = speakerTimerEl.querySelector("#timer");
const speakerNameEl = speakerTimerEl.querySelector("#active-user");
const waitingContainer = document.querySelector("#waiting-member");
...
function appendSystemMessage(text) {
  /*chatContainers.forEach((container) => {
    insertMessage(container.children[0], { username: "System", text });
  });*/
}
...
function handleSpeakerMic(micMuted) {
  speakerMic.innerHTML = `<i class="fas ${
    micMuted ? "fa-microphone-slash" : "fa-microphone"
  }"></i>`;
}
...
function toggleMic(muteStatus) {
  try {
    ...
    socket.emit("speech-muted", muteStatus.mute);
...
function toggleMicWarning(show) {
  const warningMsg = document.querySelector(".mic-message");
  if (show) warningMsg.classList.remove("hidden");
...
socket.on("speech-muted", handleSpeakerMic);
...
socket.on("conference:joined", ({ user, isActiveUser }) => {
  //HIDE WAITLIST SCREEN AND DISPLAY CONFERENCE SCREEN
  document.querySelector(".waitlist-room").style.display = "none";
  document.querySelector(".conference-room").style.display = "block";
...
socket.on("error", (payload) =>
  showMessage({ status: "error", message: payload.message || payload })
);
...
//WAITLIST SOCKET
socket.on("waiting:count", (number) => {
  state.waiting += Number.parseInt(number);
  waitingContainer.innerHTML =
    state.waiting > 0 ? `(${state.waiting} Waiting)` : "";
});
socket.on("waitlist:users", renderWaitListUsers);
socket.on("waitlist:user:joined", updateUser);
socket.on("waitlist:user:disconnect", removeUser);
socket.on("waitlist:rejoin", (payload) => {
  socket.emit("conference:join", payload);
});
