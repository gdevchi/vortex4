...
function initiateConferenceSocket(io, socket, roomId, account) {
  ...
  socket.on("user-joined", async ({ username }) => {
    try {
      ...
      if (userLength >= MAX_ROOM_LIMIT || activeRejoining[roomId]) {
        ...
        socket.to(roomId).emit("waiting:count", 1);
      } else {
        ...
        const roomUsers = (await userService.getTotalUser(roomId)) - 1;
        payload.user.index = roomUsers;
        //5. Add user in conference room
        socket.join(roomId);
        //6. Trigger user join
        socket.emit("conference:joined", payload);
        //7. Send waiting count to new user
        const waitListCounts = await waitlistService.totalWaitLists(roomId);
        socket.emit("waiting:count", waitListCounts);
        //8. Emit user payload to all users in room
  ...
  socket.on("speech-muted", async (micMuted) => {
    await userService.update(socket.id, { micMuted });
    io.to(roomId).emit("speech-muted", micMuted);
  });
        ...
        if (waitListUser) {
          ...
          const payload = {
            user: {
              ...
              roomId,
              speechDisabled: false,
              index: (await userService.getTotalUser(roomId)) - 1,
  ...
  socket.on("conference:join", async (payload) => {
